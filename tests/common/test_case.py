...
class ImageTestCase(TestCase):
    ...
    def assertImageEqual(self, input_image: Image.Image, expect_image: Image.Image, threshold=0.95):
        self.assertGreaterEqual(compute_normalized_ssim(input_image, expect_image), threshold)

    def assertImageEqualAndSaveFailed(self, input_image: Image.Image, expect_image_path: str, threshold=0.95):
        """
...
class VideoTestCase(TestCase):
    ...
    def assertVideoEqual(
        self, input_video: List[Image.Image], expect_video: VideoReader, threshold=0.95, fps: int = 15
    ):
        ssim_list = []
